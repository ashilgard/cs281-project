...
In the above algorithm, parameters include user and task specific biases, the global bias, the latent vectors for tasks, and the latent vectors for specific users as well as user features. In \citep{kula2015metadata}, the user feature latent vectors were trained at the same time as user specific latent vectors. However, for reasons described below, we find that it makes more sense to train  the latent variable model first only for user feature latent vectors. Then, after locking the user feature latent vectors and a corresponding set of parameters: $\{t_{1},g_{1}, \mu_{1j}\}$, we then train a set of user specific latent vectors and a second set of parameters $\{t_{2},g_{2}, \mu_{2j}\}$. Reports for new users are predicted as $$
  p_{i,j} = \textrm{tanh}\left(u_i^{T}t_{1j} + g_{1} + \mu_{1j}\right)
 $$ where $u_{i}$ is formed as described in Section \ref{ssec:non-task}. Reports for all other users are predicted as $$p_{i,j} = \textrm{tanh}\left( \mu_{i_M}\left[u_i^{T}t_j + g + \mu_j\right] + \mu_{i_A}\right)$$ where $\{t_{j}, g, \mu_{j}$ are formed through a respective summation of $\{t_{1},g_{1}, \mu_{1j}\}$ and $\{t_{2},g_{2}, \mu_{2j}\}$ and $u_{i}$ is formed as described in Section \ref{ssec:non-task}. 

There are two reasons that we train the user feature vectors before the individual user biases. First, while the model incorporating user feature biases is clearly more general than vanilla matrix factorization, it has more parameters and can be sensitive to the learning rate. In particular, we found that training all at once provided a worse RMSE on the predicted report values than vanilla matrix factorization, with the former having a RMSE of 0.33 and the latter having an RMSE of 0.28. This is most likely due to a too high learning rate, but the model already takes a significant time to converge at the high learning rate. If we train the user feature vectors first, and then the individual user latent vectors, the final RMSE was 0.25. 
